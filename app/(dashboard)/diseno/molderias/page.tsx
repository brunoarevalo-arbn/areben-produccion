...
export default async function MolderiasPage() {
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        <span className="text-xs font-bold uppercase tracking-widest text-stone-400">Diseño</span>
        <h1 className="text-2xl font-bold text-stone-900 mt-1">Catálogo de Molderías</h1>
        <p className="text-stone-500 text-sm mt-1">{items.length} moldería{items.length !== 1 ? 's' : ''} registrada{items.length !== 1 ? 's' : ''}</p>
      </div>
      <CatalogoManager

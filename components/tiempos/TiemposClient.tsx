...
export function TiemposClient({ usuario, ordenesIniciales }: Props) {
  ...
  useEffect(() => {
    fetch('/api/tiempos/corrida')
      .then((r) => (r.ok ? r.json() : []))
      .then((d) => setCorridas(Array.isArray(d) ? d : []))
  ...
  return (
    <div className="flex flex-col h-screen bg-stone-50">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        {/* Todo lo que sigue DESLIZA: clavado arriba, le comía la pantalla a la
            tablet. Lo relevado ya no vive acá — al cerrar, la corrida deja su
